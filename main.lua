...
function love.load()
    ...
    local dirs = love.filesystem.enumerate("games")

    for index, dirname in ipairs(dirs) do
        local main = dirname .. '/main.lua'
        table.insert(exf.available, dirname);
        exf.list:add(dirname)
    end
    exf.list:done()  
    exf.resume()
end
...
function exf.toggleFullScreen()
    fullscreen = not fullscreen
    love.graphics.setMode(800, 600, fullscreen)
end
...
function exf.draw()    
    ...
    love.graphics.print("Learn With Me", 50, 50)
    love.graphics.print("Usage", 500, 50)

    love.graphics.setFont(exf.smallfont)
    love.graphics.print("Browse and click on the game you \nwant to run. To return to this screen,\npress escape.\n", 500, 80)
...
function exf.start(item)
    ...
    -- Load game
    local file = 'games/' .. item .. '/main.lua'
    love.filesystem.load(file)()	
    exf.clear()
    love.graphics.setCaption(item)
...
function exf.resume()
    ...
    love.mouse.setVisible(true)
    love.graphics.setCaption("Learn With Me")
end
...
function List:draw()
    ...
    for i = start_i,end_i do
        ...
        love.graphics.print(self.items[i], x+10, y+16)
